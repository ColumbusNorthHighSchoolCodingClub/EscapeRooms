...
public class Location implements Comparable
{
    ...
    /**
     * Returns the actual direction in degrees from this location toward another location.
     * @return the actual direction from this location toward <code>target</code>
     */
    public int getActualDirectionToward(Location target) {
      int dx = target.getCol() - getCol();
      int dy = target.getRow() - getRow();
      // y axis points opposite to mathematical orientation
      int angle = (int) Math.toDegrees(Math.atan2(-dy, dx));
    ...
    {
        if(this.getRow() < 0 || this.getRow() >= Board.NUM_ROWS)
            return false;
        if(this.getCol() < 0 || this.getCol() >= Board.NUM_COLS)
            return false;
        return true;
    }
