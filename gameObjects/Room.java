...
/**
 * A Room is a puzzle in this game. Each room has an inventory of Items, and can
 * give hints.
 *
...
public class Room {
    ...
    public void onMousePress(Point p) {
        for (int index = 0; index < items.size(); index++) {
            Item i = items.get(index);
            if (i.getHitBox().contains(p)) {
                i.reactToMousePress(p);
            }
        }
    }

    public void onMouseRelease(Point p) {
        for (int index = 0; index < items.size(); index++) {
            Item i = items.get(index);
            i.reactToMouseRelease(p);
        }
    }
    ...
    public void draw(Graphics g, ImageObserver io) {
        for (Item i : items) {
            i.updateMouseCoords(mouseX, mouseY);
            i.draw(g, io);
        }
    }
